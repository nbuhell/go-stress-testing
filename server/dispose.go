...
func DisposeSession(concurrency, totalNumber uint64, sessionNumber int, sessionkey string, request *model.Request) {
	...
	for i := uint64(0); i < concurrency; i++ {
		...
		case model.FormTypeHttp:
			...
			go golink.HttpSession(i, ch, totalNumber, sessionNumber, sessionkey, &wg, request)
